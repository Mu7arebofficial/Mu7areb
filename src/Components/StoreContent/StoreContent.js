...
const StoreContent = (props) => {
    ...
    return (
            <div className='Products row p-0 m-0 pb-5  '>
                        ...
                        { isLoading ? 'Loading':  data?.data?.categories.map((item) => {
                            return (
                                <div className='mb-2 sub-cat' key={item._id}>
                                    <input name='cat' id={item._id} type='radio' value={item.name} onClick={() => setSelectedSubCategory(item.name)} className='me-2' />
                ...
                <ProductsList searchKey={searchKey ? searchKey : ''} subCategory={selectedSubCategory} category={params.category} priceFilter={priceFilter} priceOrder={priceOrder} />
        </div>
    )
}
