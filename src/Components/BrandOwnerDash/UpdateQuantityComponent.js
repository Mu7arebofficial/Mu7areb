...
const UpdateQuantityComponent = ({sizes , setUpdateQuantityView}) => {
    ...
    const sendQuantity = async () => {
        const res = await axios.post(`https://e-commerce-hh3m.onrender.com/api/brand/updateQuantity/${sizes.id}?size=${size}&quantity=${sizeQuantity}` ,"",{
            headers: {
  ...
  return (
    ...
    <div className='updateSizes d-flex justify-content-between align-items-center p-5'>
       ...
       <input onChange={(e) => setSizeQuantity(e.target.value)} type='number' min={1}  />
       <button className='saveButton' onClick={sendQuantity}>Save</button>
    </div>
    <ToastContainer
                position="top-left"
                autoClose={2500}
