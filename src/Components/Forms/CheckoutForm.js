      ...
      {orderIsDone ? (
        <div className="checkout-msg">
          <div className="recieptProdcuts">
            {products &&
              products.map(ele => {
                return (
                  <div key={ele._id} className="recieptProduct d-flex justify-content-between">
                    <p>{ele.name}</p>
          ...
          <button
            className="btn btn-primary checkout-buttons d-flex "
            onClick={() => props.setExit(false) + dispatch(Reset())}>
            Close
          </button>
        </div>
      ) : (
        <>
          <form className="checkout-form" onSubmit={formHandler}>
            ...
            <input value={props?.user?.email} ref={emailRef} className="w-100 p-2 mb-4" type="email" />
            <label className="mb-3 h5">Your Phone Number</label>
            ...
            <div className="checkout-buttons d-flex ">
              ...
              <button onClick={() => props.setExit(false)} className="btn btn-danger  me-3">
                Close
              </button>
            </div>
          </form>
