...
const ProductCustomization = () => {
    ...
    const formHandler = async (e) => {
        ...
        await axios.patch(`https://e-commerce-hh3m.onrender.com/api/products/${id}`, {
...
<>
            ...
            <h1>Customize product </h1>
                <div>
                    <label>Name</label><br />
                    <input value={name || data?.data?.choosenProduct?.name } onChange={(e) => setName(e.currentTarget.value)} className='mt-1 p-1' type='text' placeholder='New Name'  />                    
                </div>
                
                <label className='mt-3'>Price: ${price || data?.data?.choosenProduct?.price }</label><br />
                <input className='mt-1  p-1' value={price || data?.data?.choosenProduct?.price} onChange={(e) => setPrice(e.currentTarget.value)} type='number' placeholder='New Price' /><br />
                ...
                <div className='w-300 d-flex justify-content-between align-items-start dash-cat'>
                    <div>
                            ...
                            <input onChange={subCategoryHandler} type='radio' className='me-2 ms-2' name='sub-cat' id='t-shirt' /><label  className=' mt-2'htmlFor='T-shirts'>T-shirts</label><br />
                            <input onChange={subCategoryHandler} type='radio' className='me-2 ms-2' name='sub-cat' id='running' /><label  className=' mt-2'htmlFor='Track pants'>Track pants</label><br />
                        </div>
                        <div>
                ...
                <textarea value={description || data?.data?.choosenProduct?.description} onChange={(e) => setDescription(e.currentTarget.value)} placeholder='New Description' className='mt-0 p-2' rows="4" cols="30" width='200' height='150'></textarea><br />
                <div className='productCustomizeImage d-flex flex-column justify-content-start mt-4 align-items-center'>  
                        {image1 ? <img src={URL.createObjectURL(image1)} alt='img1' />: <img src={data?.data?.choosenProduct?.images[0]?.url} alt='img1'/>}
                        <label  className='btn  btn-danger mt-4' htmlFor='image-upload1'>change</label>                              
                </div>
                <div className='productCustomizeImage d-flex flex-column justify-content-start mt-4 align-items-center'>  
                        {image2 ? <img src={URL.createObjectURL(image2)} alt='img2' />: <img src={data?.data?.choosenProduct?.images[1]?.url} alt='img'/>}
                        <label  className='btn  btn-danger mt-4' htmlFor='image-upload2'>change</label>                              
                </div>
                ...
                <br />
                <button type='submit'  className='btn btn-primary btn-lg mt-3 p-2 w-300'>Send</button>
            </form>
            <ToastContainer
                position="top-left"
                autoClose={1000}
